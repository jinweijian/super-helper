...
interface KnowledgeRagSearchQuery extends KnowledgeSearchQuery {
  retrievalLimit?: number;
  embedding?: {
    provider: KnowledgeEmbeddingQueryProvider;
    limit?: number;
  };
  rerank?: {
...
export async function searchKnowledgeWithRag(input: KnowledgeRagSearchQuery): Promise<KnowledgeEvidencePack> {
  const retrieval = await import('../retrieval/legacy-rag.js');
  return retrieval.searchKnowledgeWithRag(input);
}
