...
Define the multi-agent configuration registry that pairs each Agent entry with explicit role, prompt, model provider, and tool surface so the runtime can resolve a diagnostic Agent without parsing loose YAML or hidden defaults.
