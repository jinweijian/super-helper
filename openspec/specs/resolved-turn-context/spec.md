...
Define the bounded resolved turn context that Preflight builds once per dispatched turn so that downstream knowledge, retrieval, deep query, and worker stages consume the same resolved query, facts, claims, hypotheses, and unknowns while the raw latest message stays available only for audit and UI rendering.
